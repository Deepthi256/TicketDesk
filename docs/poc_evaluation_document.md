...
|---|---|---|---|---|
| **Member 1** | **Cloud Architect / Lead DevOps** | **IaC & Core Networking** | `terraform/vpc.tf`<br>`terraform/subnets.tf`<br>`terraform/security_groups.tf`<br>`terraform/main.tf` | Designed Multi-AZ VPC network topology (`10.0.0.0/16`), public/private subnet segmentation across 2 AZs, IGW, NAT Gateway, and 3-tier security group perimeters (`sg-alb`, `sg-ecs`, `sg-rds`). |
| **Member 2** | **Backend & Container Engineer** | **Backend API & Containerization** | `Dockerfile`<br>`.dockerignore`<br>`Backend/app/`<br>`terraform/ecr.tf`<br>`terraform/ecs.tf` | Built multi-stage Dockerfile running as non-root `appuser` (UID 10001); configured ECR immutable repository and ECS Fargate Task Definition with Secrets Manager dynamic credential resolution. |
| **Member 3** | **Database & Security Specialist** | **Database & Secrets Management** | `terraform/rds.tf`<br>`terraform/secrets_ssm.tf`<br>`Backend/app/database.py` | Provisioned Multi-AZ RDS PostgreSQL 15 instance in private subnets with KMS encryption at rest, AWS Secrets Manager password auto-generation, SSM Parameter Store config, and SQLAlchemy connection pooling. |
| **Member 4** | **Frontend & CDN Engineer** | **Frontend SPA & CloudFront CDN** | `Frontend/src/`<br>`terraform/s3_frontend_cloudfront.tf`<br>`scripts/deploy_frontend.py` | Developed React 18 TypeScript frontend, configured private S3 bucket storage with CloudFront CDN Origin Access Control (OAC), and `/api/*` path routing rules to Application Load Balancer. |
| **Member 5** | **Serverless & Automation Engineer** | **Lambda & CI/CD Pipeline** | `lambda/thumbnail_generator.py`<br>`terraform/s3_attachments_lambda.tf`<br>`.github/workflows/deploy.yml` | Implemented S3 presigned direct upload workflow, AWS Lambda serverless thumbnail generator with Pillow, and GitHub Actions full-stack CI/CD pipeline (pytest, Gitleaks, ECR push, ECS deploy, CDN invalidation). |
| **Member 6** | **Observability & QA Engineer** | **Monitoring, Alarms & Testing** | `terraform/observability.tf`<br>`Backend/tests/`<br>`docs/cost_report.md`<br>`docs/deployment_checklist.md` | Configured CloudWatch Log Group (14-day retention), 4-widget Operations Dashboard, 3 CloudWatch metric alarms with SNS alerts, automated smoke test suite, 20-user concurrency test, and one-page cost report. |
